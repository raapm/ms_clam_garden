...
write.table(y$genes, file = " CG_DG_background_gene_list_May_2021.txt")
...
my.contrasts <- makeContrasts(
  HvsLSurv = (GroupCG.A + GroupRef.D) - (GroupCG.E + GroupRef.F),
  HvsMSurv = (GroupCG.A + GroupRef.D) - (GroupCG.C + GroupRef.B),
  MvsLSurv = (GroupCG.C + GroupRef.B) - (GroupCG.E + GroupRef.F), levels = design)
#
fit <- glmQLFit(y, design)
...
cpm(y)[o[1:23200],]
tab <- cpm(y)[o[1:23200],]
write.table(tab, file = "CG_DG_HMLSurvival_ANOVA_cpm_top genes_May19.2021.txt")


tab <- topTags(anov, n = 23200)
write.table(tab, file = "CG_DG_HMLSurvival_DE_Anova_May19.2021.txt")
#
tab <- cpm(y)[o[1:150],]
#
