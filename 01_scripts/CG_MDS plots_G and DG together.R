...
targets <- targets[ ,1:28]
targets

# Combine all the experimental factors into one combined factor
Group <- factor(paste(targets$tissue, targets$beach, sep = "."))
...
#
Tissue <- factor(targets$tissue, levels =c("Gill","DG"))
Beach <- factor(targets$beach, levels = c("A","C", "E", "B", "D", "F"))
Type <- factor(targets$type, levels = c("CG", "NW"))
...
points <- c(0,15)
#
plotMDS(y, pch = points[Tissue], ylab = "Leading logFC dim 2",
        xlab = "Leading logFC dim 1", main = "")
legend("topright", legend = levels(Tissue), pch = points, ncol =2)
# 
#
# MDS plot "gene.selection" option
plotMDS(y, gene.selection = "common", pch = points[Tissue], ylab = "Leading logFC dim 2",
        xlab = "Leading logFC dim 1", main = "")
legend("topright", legend = levels(Tissue), pch = points, ncol =2)
# 